...
``DryState`` is a ``LogitsProcessor``, one entry in
``Sampler.logits_processors`` between ``penalties_state`` and
``bad_words_state``. The match computation lives in
``vllm.v1.sample.dry_core``; windows are gathered directly from the
...
class DryState(LogitsProcessor):
    ...
    def apply(self, logits: torch.Tensor, ctx: LogitsContext) -> torch.Tensor:
        ...
        seq_lens_np = ctx.seq_lens_upper_bound_np
