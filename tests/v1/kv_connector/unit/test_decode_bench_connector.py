...
def test_decode_bench_connector_fills_unique_blocks_once(reqs_blocks, expected_blocks):
    ...
    def _counting_fill(cache, block_ids, fill_mean, fill_std):
        block_fill_calls.append((id(cache), tuple(block_ids)))
        return orig_fill(cache, block_ids, fill_mean, fill_std)
...
def test_decode_bench_connector_fills_shared_state_tensors_once():
    ...
    worker = connector.connector_worker
    assert worker is not None
    orig_fill = worker._fill_state_tensor

    def _counting_fill(cache, fill_mean, fill_std):
        state_fill_ids.append(id(cache))
        return orig_fill(cache, fill_mean, fill_std)

    worker._fill_state_tensor = _counting_fill  # type: ignore[method-assign]
    _bind_and_fill(
        connector,
        {
            "r0": (([0, 1],), 32),
