...
containment resolution, the sampler's DryState logits processor, and
...
# ---------------------------------------------------------------------------
# V2-runner DryState logits processor
# ---------------------------------------------------------------------------
...
def _apply(state, logits, idx_mapping_np, seq_lens_np, *, expanded_logits=None):
    ...
    rows = logits.shape[0]
    if expanded_logits is not None:
        assert (rows != idx_mapping_np.shape[0]) == expanded_logits
    idx_mapping = torch.from_numpy(idx_mapping_np).to(logits.device)
    # Unexpanded batches are one row per request; an expanded batch repeats a
    # request's slot across its draft rows.
    expanded = (
        idx_mapping
        if rows == idx_mapping_np.shape[0]
        else idx_mapping.repeat_interleave(rows // idx_mapping_np.shape[0])
    )
    ctx = LogitsContext(
        expanded_idx_mapping=expanded,
        idx_mapping=idx_mapping,
        idx_mapping_np=idx_mapping_np,
        expanded_local_pos=torch.zeros(rows, dtype=torch.int64, device=logits.device),
        input_ids=torch.zeros(rows, dtype=torch.int32, device=logits.device),
...
def test_v2_worked_example_and_window_bounds():
    ...
    _apply(state, logits, idx_mapping, seq_lens, expanded_logits=False)
...
def test_v2_penalty_last_n_window():
    ...
    logits = torch.zeros(1, 32, device=DEVICE)
    _apply(state, logits, np.array([0]), np.array([20]), expanded_logits=False)
...
def _validate_with_runner(params, *, use_v2_model_runner):
    ...
    stub = SimpleNamespace(
        ...
        # Built from the logits processors' validate_params hooks in
        # InputProcessor.__init__; no custom processor is loaded here.
...
def test_dry_rejected_on_v1_model_runner():
    # DRY is implemented once, in the DryState logits processor the V2 model
